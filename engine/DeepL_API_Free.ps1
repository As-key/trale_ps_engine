...
If($source_language -eq "AUTO"){
  $ret = Invoke-WebRequest -Body @{ auth_key="$($auth_key)"; text="$($source)"; target_lang="$($target_language)" } https://api-free.deepl.com/v2/translate
}Else{
  $ret = Invoke-WebRequest -Body @{ auth_key="$($auth_key)"; text="$($source)"; source_lang="$($source_language)"; target_lang="$($target_language)" } https://api-free.deepl.com/v2/translate
}
